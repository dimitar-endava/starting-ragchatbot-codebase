...
class TestSystemHealth:
    ...
    def test_rag_system_initialization(
        self, mock_anthropic, mock_embedding_func, mock_chroma_client
    ):
...
It builds upon the previous lesson.
    ...
    @patch("chromadb.utils.embedding_functions.SentenceTransformerEmbeddingFunction")
    @patch("anthropic.Anthropic")
    def test_real_world_query_flow(
        self, mock_anthropic, mock_embedding_func, mock_chroma_client
    ):
        ...
        mock_catalog_collection.get.return_value = {
            "metadatas": [{"lessons_json": "[]"}]  # Empty lessons for simplicity
        }
    ...
    def test_actual_chromadb_directory(self):
        ...
        try:
            ...
            if not os.path.exists(parent_dir):
                pytest.fail(
                    f"Parent directory for ChromaDB doesn't exist: {parent_dir}"
                )
            ...
            if not os.access(chroma_path, os.W_OK):
                pytest.fail(
                    f"No write permissions for ChromaDB directory: {chroma_path}"
                )
